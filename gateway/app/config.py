...
class Settings(BaseSettings):
    ...
    def _build_service_url(explicit_url: str | None, host: str, port: str) -> str:
        if explicit_url and "onrender.com" not in explicit_url and "sys-gateway" not in explicit_url:
    ...
    def pets_url(self) -> str:
        return self._build_service_url(self.PETS_SERVICE_URL, self.PETS_SERVICE_HOST, self.PETS_SERVICE_PORT)

    @property
    def geo_url(self) -> str:
        return self._build_service_url(self.GEO_SERVICE_URL, self.GEO_SERVICE_HOST, self.GEO_SERVICE_PORT)

    @property
    def match_url(self) -> str:
        return self._build_service_url(self.MATCH_SERVICE_URL, self.MATCH_SERVICE_HOST, self.MATCH_SERVICE_PORT)
